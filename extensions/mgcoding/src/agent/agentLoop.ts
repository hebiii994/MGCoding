...
function toolSystemPrompt(): string {
	const list = TOOL_SPECS.map(t => `- ${t.name}: ${t.description} args: ${t.args}`).join('\n');
...
export async function runAgent(
	...
	for (let i = 0; i < MAX_ITERATIONS; i++) {
		...
		const reply = await complete(registry, messages, sys, signal);
		...
		// testo eventuale prima del blocco tool (ragionamento)
