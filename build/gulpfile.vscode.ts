...
function hasAuthenticodeSignature(filePath: string): Promise<boolean> {
	return new Promise((resolve, reject) => {
		...
		proc.on('error', (err: NodeJS.ErrnoException) => {
			// signtool.exe (Windows SDK) non disponibile: nelle build locali non firmate
			// non serve rimuovere la firma, quindi trattiamolo come "nessuna firma".
			if (err.code === 'ENOENT') {
				resolve(false);
			} else {
				reject(err);
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		const deps = (await Promise.all([
			glob('**/*.node', { cwd, ignore: ['extensions/node_modules/@parcel/watcher/**', '**/*linux*/**', '**/*darwin*/**'] }),
			glob('**/rg.exe', { cwd }),
